...
One can see that for language_eq=False the estimate 6.04 is not withing the boundaries of language_eq=True (low=6.1 and upper=6.6).
This would indicate that the difference in vividness for native and foreign language is significant but very low.
...
The amount of answered sentences for each modality varies. Therefore, we think it might be problematic to just use the mean() function to calculate the overall estimate + lower and upper bound. We are not sure if the result would still be accurate and correct.
...
Again we cannot make any real statement about this hypothesis, as we do not know if using the mean over the estimate, lower bound and upper bound is correct.
Nevertheless, we want to present the results that would come out if it is correct:
...
=======
modality_plot
>>>>>>> 38497cf18fbcdbd84bffa7e6a95ed2da0da8c23b
```
...
This hypothesis states that the olfactory estimate and gustatory ratings are significantly lower than the other modalities. In the given graph one can see that this does only hold for olfactory when also considering the language type. Within language_eq = Flase (orange) the olfactory ratings are significantly smaller than the other modalities. The same holds for language_eq = true. However, this is not the case for the gustatory modality. The gustatory modality is only significantly lower than auditory and motor modality but not organic, tactile and visual.
```{r}
modality_plot
```
If we compare this to the modality_plot that uses the mean to average over the language we can see a different result. Whereas olfactory shows the same result like the modality-language plot, gustatory shows a different result.
In the modality-language plot gustatory is only significantly lower for native language and not for foreign language, whereas in the modality plot gustatory is indicated as overall significantly lower.
