...
```{r}
d_individual_summary %>% ggplot(aes(x=education, fill = gender)) + theme_bw() + geom_bar()
d_individual_summary %>% group_by(education,gender) %>% summarise(education_count = n())
...
```{r}
d_individual_summary %>%  group_by(exp_language) %>% summarise(language_exp_count = n())
...
### Trying alternative Methods/Mpdels 
...
modality_effects
#compute the difference for each cell
result2 <- language_effects3[,2:4] - modality_effects[,2:4]
result2
...
#testing
manu_language<-wilcox.test(ch$response ~ ch$language_eq)
...
The difference is very small! There are differences but they are not meaningfull. 
